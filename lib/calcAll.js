...
export function calcAll(baseSalary, contract, params) {
  ...
  // החזר הוצאות רכב שירות: נטו לעובד (המעסיק מגלם את המס) — אינו חלק מהברוטו
  const carReimbursement = (cp.isCar && !companyCar) ? cp.value : 0;
  ...
  const carGross = effectiveTravelPay; // רק קצובת נסיעה — החזר הוצאות רכב מוצג בנפרד
  ...
  // החזר הוצאות רכב שירות: נטו לעובד (גילום ע"י המעסיק) — נכנס לערך הכלכלי
  if (carReimbursement > 0) totalEconomicValue += carReimbursement;
