...
export function calcAll(baseSalary, contract, params) {
  ...
  // החזר הוצאות רכב שירות: חלק משתנה (340 ₪) — ברוטו ממוסה; חלק קבוע — פטור ממס
  const carVariable = (cp.isCar && !companyCar) ? (cp.variable || 0) : 0;
  const carFixed    = (cp.isCar && !companyCar) ? (cp.fixed    || 0) : 0;
  const carGross = effectiveTravelPay + carVariable; // carVariable נכנס לברוטו
  ...
  // netSalary = נטו כולל — ברוטו שכר בניכוי ניכויים + החזר קבוע רכב (פטור ממס)
  const netSalary = totalGross - totalDeductions + carFixed;
  ...
  return {
    ...
    effectiveTravelPay, carVariable, carFixed, ashalPay,
  };
}
