...
// שיעור מס שולי לחישוב גילום החזר הוצאות רכב
function getMarginalTaxRate(income) {
  if (income <= 7010)  return 0.10;
  if (income <= 10060) return 0.14;
  if (income <= 19000) return 0.20;
  if (income <= 25100) return 0.31;
  if (income <= 46690) return 0.35;
  if (income <= 60317) return 0.47;
  return 0.50;
}
...
export function calcAll(baseSalary, contract, params) {
  ...
  // החזר הוצאות רכב שירות: הסכום בחבילה הוא נטו לעובד.
  // המעסיק מגלם לפי המס השולי → הסכום הגלום נכנס לברוטו, העובד מקבל את הנטו לאחר מס.
  const carNetValue = (cp.isCar && !companyCar) ? cp.value : 0;
  const incomeBeforeCar = baseSalary + overtimePay + effectiveTravelPay + maonotPay + oncallPay + premiumPay;
  const carMarginalRate = getMarginalTaxRate(incomeBeforeCar);
  const carGrossedUp = carNetValue > 0 ? carNetValue / (1 - carMarginalRate) : 0;
  const carGross = effectiveTravelPay + carGrossedUp;
  ...
  // netSalary = נטו מזומן (ברוטו שכר בלבד בניכוי כל הניכויים)
  ...
  // שווי שימוש ברכב נכנס לערך הכלכלי (לא לברוטו)
  if (companyCarImputationNet > 0) totalEconomicValue += companyCarImputationNet;
  // carGrossedUp כבר בתוך netSalary (דרך totalGross) — אין צורך להוסיף שוב
  ...
  return {
    ...
    effectiveTravelPay, carGrossedUp, carNetValue, carMarginalRate, ashalPay,
  };
}
