...
export function calcAll(baseSalary, contract, params) {
  ...
  // החזר הוצאות רכב שירות — שלושה רכיבים:
  // 0700 קבועות ברוטו: נכנס לבסיס המס | 0750 קבועות נטו + משתנות: פטור ממס
  const carGrossFixed   = (cp.isCar && !companyCar) ? (cp.grossFixed   || 0) : 0;
  const carNetFixed     = (cp.isCar && !companyCar) ? (cp.netFixed     || 0) : 0;
  const carNetVariable  = (cp.isCar && !companyCar) ? (cp.netVariable  || 0) : 0;
  const carNet = carNetFixed + carNetVariable; // סה"כ פטור ממס
  const carGross = effectiveTravelPay + carGrossFixed; // רק 0700 נכנס לברוטו
  ...
  // netSalary = נטו כולל — ברוטו שכר בניכוי ניכויים + החזר רכב פטור ממס (0750 + משתנות)
  const netSalary = totalGross - totalDeductions + carNet;
  ...
  return {
    ...
    effectiveTravelPay, carGrossFixed, carNetFixed, carNetVariable, carNet, ashalPay,
  };
}
