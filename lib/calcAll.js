...
// מחשב מס הכנסה חודשי לפי מדרגות 2025
...
export function calcAll(baseSalary, contract, params) {
  ...
  const effectiveTravelPay = companyCar ? 0 : travelPay;
  ...
  const totalGross = baseSalary + overtimePay + carGross + maonotPay + companyCarImputation + oncallPay + premiumPay;
  ...
  const localityBenefit = localityGroupData
    ? Math.min(Math.min(totalGross, localityGroupData.monthlyCeiling) * localityGroupData.rate, localityGroupData.maxMonthlyBenefit)
    : 0;
  let incomeTax = Math.max(0, calcIncomeTax(totalGross) - taxCreditValue - localityBenefit);
  ...
  const clothing = 206;
  const gifts = 100;
  const cibus = 22 * 20; // 22 ₪ × 20 ימי עבודה
  const phone = 130;
  ...
  return {
    ...
    totalGross, points, miluimPoints, taxCreditValue, localityBenefit,
    ...
    cibus, phone, clothing, gifts,
    hourlyRate: gradeHourlyRate, recoveryPay, recoveryDays, travelAllowance,
    companyCarImputation, oncallPay, premiumPay,
  };
}
